...
The function should removes three items in the array starting with 
...
As you sharpen the pencil, the string will become shorter and shorter, starting by removing 
the first letter.
...
Write a function name wordsToCharList that, given a string as input,
 returns a new array where every element is a character of the input
  string.
...
w. Rather than taking the entire recipe, you only want a list of the item names.

Write a function named listFoods that takes in the recipe and returns a new array 
of the food items without any amount or units. Just the name. For example, '1 cup 
flour' will return 'flour'.
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(value => {
    ...
    let item = value.slice(value.indexOf(' ') + 1);
