...
const toLastNames = people => {
  ...
  let newArry = [];
  people.map(value => {
    newArry.push(value.firstName + ' ' + value.lastName);
  })
  return newArry;
};
...
Write a function named addValues that, given an array 
of numbers as input, uses reduce to add the values in
 the array.
...
const addPurchases = (arr) => {
  ...
  return arr.reduce((add, val) => {
    return add = add + val.purchasePrice;
  }, 0);
};
...
Write a function named returnNames that, given the Star
 Wars data, below, uses reduce to return an array 
...
const returnNames = (arr) => {
  // Solution code here...
  const newArry = arr.reduce((star, val) => {
    star.push(val.name);
    return star;
  }, []);
  return newArry;
...
Write a function named reversedString that takes in a
 string and returns a string with the letters in 
 reverse order.
